# Module 2 - Create Crypto Currency
...
def mine_block():
    ...
    blockchain.add_transaction(sender = node_address, reciever= 'Me', amount= 1)
...
def is_valid():
    ...
    if is_valid:
        ...
        response = {'message': 'Problem! The Chain is NOT valid'}
    return jsonify(response), 200
...
def add_transaction():
    ...
    trabnsaction_keys = ['sender', 'reciever', 'amount']
    if not all (key in json for key in trabnsaction_keys):
        return 'Transaction Details are missing.', 400
...
def connect_node():
    ...
    response = {'message': 'All Nodes are now connected. The Blockchain now contains:',
...
def replace_chain():
    ...
    if is_chain_replaced:
        response = {'message': 'Chain was replaced by longer version!',
                    'new_chain': blockchain.chain}
    else:
        response = {'message': 'All good,  Chain was the largest one.',
